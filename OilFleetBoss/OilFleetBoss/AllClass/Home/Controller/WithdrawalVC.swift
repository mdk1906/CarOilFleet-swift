...
import Alamofire
import SwiftyJSON
class WithdrawalVC: MDKBaseListViewController {
    var data  = [WithdrawalModel]()
    var typeStr :String?
    var money:String?
    var count :String?
    ...
    */
    ...
    func tixianAll () {
        ...
        let params:Dictionary = ["custId":custId,"fromType":"3","total":m.description]
        print("url = ",url)
        print("params = ",params)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: cellID) as! WithdrawalTableViewCell
                ...
                cell.WithdrawalModel = data[indexPath.row]
        return cell
    }
    ...
    func getStateData()  {
        ...
        let params:Dictionary = ["rewardType":"3","custId":custId]
        print("url = ",url)
        print("params = ",params)
        Alamofire.upload(multipartFormData: { (multipartFormData) in
            for (key, value) in params {
...
//                                self.getQRCode()
    ...
    func getData()  {
        self.data = [WithdrawalModel]()
        let url = BASE_URL + k_commissionList
                    ...
                    if let value:NSDictionary = response.result.value as! NSDictionary? {
                        ...
                        if dict["ret"].intValue == 1 {
                            ...
                            if let items = dict["data"].arrayObject {
                                for item in items {
                                    print(item)
                                    let post = WithdrawalModel(dict: item as! [String: AnyObject])
