...
import Alamofire
import SwiftyJSON
class IntegralWithdrawalVC: MDKBaseListViewController {
    var data  = [IntegralWithdrawalModel]()
    var money:String?
    var count :String?
    ...
    func tixianAll () {
        ...
        let params:Dictionary = ["custId":custId,"fromType":"2","total":m.description]
    ...
    func nextClick()  {
        if money == "" {
            ...
            let m :Int = Int(Double(mon)!*100)
            let params:Dictionary = ["custId":custId,"fromType":"2","total":m.description]
//            let params:Dictionary = ["custId":custId,"fromType":"2","total":money]
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: cellID) as! WithdrawalTableViewCell
                ...
                cell.IntegralWithdrawalModel = data[indexPath.row]
        return cell
    }
    ...
    func getStateData()  {
        ...
        let params:Dictionary = ["rewardType":"2","custId":custId]
        print("url = ",url)
        print("params = ",params)
        Alamofire.upload(multipartFormData: { (multipartFormData) in
            for (key, value) in params {
    ...
    func getData()  {
        self.data = [IntegralWithdrawalModel]()
        let url = BASE_URL + k_IntegralWithdrawalList
                    ...
                    if let value:NSDictionary = response.result.value as! NSDictionary? {
                        ...
                        if dict["ret"].intValue == 1 {
                            ...
                            if let items = dict["data"].arrayObject {
                                for item in items {
                                    print(item)
                                    let post = IntegralWithdrawalModel(dict: item as! [String: AnyObject])
