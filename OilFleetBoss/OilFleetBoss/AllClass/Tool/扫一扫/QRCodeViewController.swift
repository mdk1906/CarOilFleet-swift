...
class QRCodeViewController: MDKBaseViewController , UIAlertViewDelegate, HandleTheResultDelegate, UIImagePickerControllerDelegate,UINavigationControllerDelegate{
    ...
    func initScan() {
        ...
        self.readView = QRCodeReaderView.init(frame: CGRect(x:0,y:64,width:WindowWidth,height:WindowHeight-64))
    ...
    func handleResult(_ result: String) {
        print("处理扫描结果\(result)")
        let string:NSString = result as NSString
        let substringArry = string.components(separatedBy: "/");
        
        if substringArry[0] == "oilFleet" {
            loadFailure(msg: "扫描成功")
            NotificationCenter.default.post(name: NSNotification.Name.init("saomiaoSecuess"), object: substringArry)
            self.leftEvent()
        }
        else{
//            let alertVC = UIAlertController(title: "扫描无效二维码", message: "请重新扫描", preferredStyle: UIAlertControllerStyle.alert)
//            let acSure = UIAlertAction(title: "重新扫描", style: UIAlertActionStyle.destructive) { (UIAlertAction) -> Void in
//                //重新扫描
//                self.reStartScan()
//            }
